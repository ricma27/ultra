...
module.exports = function sonosPlayMP3(fileName, volume, adjustStopMusicAtEnd=0){
	return new Promise(function(resolve, reject){
		if(shareSonos.newTrackState === 1){
			console.error('Function relaunched too fast, the old track has not been loaded yet');
			reject(1);
		}
		else{
			...
			.then((sonosDevices) => {
    			...
    			.then(function(){
    				...
    				gladys.modules.sonos.music.play({uri: 'x-file-cifs://gladys/Partage/' + fileName, deviceType : {device : sonosDevices[0]}})
    				.then(function(){			
        				gladys.modules.sonos.music.getCurrentTrack({deviceType : {device : sonosDevices[0]}})
        				.then(function(currentTrack){
                			...
                			currentTrack.duration = (currentTrack.duration*1000)+500 - adjustStopMusicAtEnd;
                		...
                		.then(function(){
            				if(trackPosition === shareSonos.newTrackPosition){ // = not sonosPlayMp3 function was launched once again
                				...
                				gladys.modules.sonos.music.removeTrackFromQueue({trackNumber : shareSonos.newTrackPosition, deviceType : {device : sonosDevices[0]}})
            			    	.then(function(){
            		...
            		.catch(() => {
						shareSonos.newTrackState = 0;
    					console.error('Don\'t find : ' + fileName + ' in folder : ./cache/sound');
    					reject(1);
            		});
