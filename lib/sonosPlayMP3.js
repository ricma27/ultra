...
var playNewTrackEndQueue = (fileName, volume, stop, arrayDevicesIDSonos) => new Promise((resolve, reject)=>{
    ...
    .then(() => {
		return gladys.modules.sonos.music.selectTrack({trackNumber : sonosShare.newTrackPosition, deviceType : {device : arrayDevicesIDSonos[0]}});
	})				
	.then(() => {
		sonosSetVolume(volume, arrayDevicesIDSonos);
