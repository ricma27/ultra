...
var playNewTrackEndQueue = (fileName, volume, arrayDevicesIDSonos) => new Promise((resolve, reject)=>{
	...
	.then((currentTrack) => {
        ...
        currentTrack.duration = (currentTrack.duration*1000);
		shareSonos.timerDeleteTrack = timeout(currentTrack.duration);
		shareSonos.timerDeleteTrack.promise	
		.then(() => waitEndTrack(arrayDevicesIDSonos[0], 2800))
		.then(() => resolve(1));
...
var waitEndTrack = (arrayDevicesIDSonos, waitTimeMax) => new Promise((resolve, reject)=>{
	...
	var checkEndTrackInterval = () => new Promise((resolve, reject) => {setTimeout(resolve, 100)});	
	var loopWaitEndTrack = function(){
		if(!endLoop && shareSonos.relaunchFunction){
		    gladys.music.getPlaying({devicetype: arrayDevicesIDSonos[0]})
		    .then((result) => {
			    if(!result.playing && shareSonos.relaunchFunction)
	    		    resolve(1);
	    	    else{
...
var restartingPlaylist = (volume, arrayDevicesIDSonos)=> new Promise((resolve, reject)=>{
	...
	setVolume(15, arrayDevicesIDSonos);
